...
function getFilters() {
  ...
  return {
    position: inputs[0].value.trim().toLowerCase(),
    seniority: inputs[1].value.trim().toLowerCase(),
    business: inputs[2].value.trim().toLowerCase(),
    location: inputs[4].value.trim().toLowerCase()
  };
}
...
function applyFilters() {
  ...
  const filtered = allData.filter(entry => {
    return (!filters.position || entry.positionName.toLowerCase().includes(filters.position)) &&
           (!filters.seniority || entry.seniority.toLowerCase().includes(filters.seniority)) &&
           (!filters.business || entry.businessName.toLowerCase().includes(filters.business)) &&
           (!filters.location || entry.location.toLowerCase().includes(filters.location));
  });

  dataBatches = splitIntoBatches(filtered, rowsPerPage);
  renderBatch(1);
}
...
document.addEventListener("DOMContentLoaded", () => {
  fetch("../data/positions.sample.json")
    ...
    .then(data => {
      ...
      // Bind input filters dynamically
      const inputs = document.querySelectorAll("input");
